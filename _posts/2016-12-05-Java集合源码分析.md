...
size：集合现在实际的大小。为什么不直接使用elementData.length，如果直接使用elementData，每次插入，删除都要操作数组的空间（代价大），所以elementData并非每个空间都实际用到，所以不能使用length表示大小。（后面的其他集合对象也都是一样的道理）
...
主要的几个常用的Collection，Map都大概看了一下源码分析，其实内部实现都是以前数据结构上学过的那些常用的数据结构，不过在看源码的时候发现很多地方都是使用了位运算，毕竟位运算效率高。而且很多地方都尽量契合二进制的一些计算规则。在看源码时候，感觉主要点就两点，数据结构的使用，位运算的使用。